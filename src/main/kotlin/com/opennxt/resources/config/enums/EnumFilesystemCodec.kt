...
object EnumFilesystemCodec : FilesystemResourceCodec<EnumDefinition> {
    ...
    fun decode(buffer: ByteBuffer): EnumDefinition {
        ...
        while (buffer.hasRemaining()) {
            ...
            when (opcode) {
                ...
                101 -> {
                    ...
                    if (resolved == null) definition.unknownKeyTypeId = type
                    else definition.keyType = resolved
                }
                102 -> {
                    val type = buffer.getSmallSmartInt()
                    val resolved = ScriptVarType.getById(type)
                    if (resolved == null) definition.unknownValueTypeId = type
                    else definition.valueType = resolved
                }
    ...
    override fun store(fs: Filesystem, id: Int, data: EnumDefinition) {
        ...
        while (true) {
            try {
                ...
                buffer.put(101)
                buffer.putSmallSmartInt(data.unknownKeyTypeId ?: data.keyType.id)

                buffer.put(102)
                buffer.putSmallSmartInt(data.unknownValueTypeId ?: data.valueType.id)
