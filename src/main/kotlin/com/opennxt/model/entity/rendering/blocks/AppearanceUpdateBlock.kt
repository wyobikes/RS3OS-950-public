...
class AppearanceUpdateBlock : UpdateBlock(UpdateBlockType.APPEARANCE) {
    override fun encode(buffer: GamePacketBuilder, viewer: WorldPlayer, entity: Entity) {
        if (entity is PlayerEntity) {
            ...
            if (lengthIsBiased()) {
                buffer.put(DataType.BYTE, DataTransformation.ADD, data.size)
            } else {
    ...
    companion object {
        fun lengthIsBiased(): Boolean =
            when (System.getProperty("opennxt.compat.appearanceLength")) {
                "plain" -> false
                "add" -> true
                else -> UpdateBlockType.experimentalBuild()
            }
    }
}
