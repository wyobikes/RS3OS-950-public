...
class WorldList(var entries: Array<WorldListEntry> = emptyArray()) {
    ...
    fun handleRequest(oldChecksum: Int, output: ConnectedClient) {
        ...
        buffer.writeByte(2)
        buffer.writeByte(if (requiresFullUpdate) 1 else 0)
    ...
    companion object {
        fun demoEntries(): Array<WorldListEntry> = arrayOf(
            WorldListEntry(
                id = 1,
                location = WorldListLocation(225, "Live"),
                flag = if (com.opennxt.config.Membership.enabled()) WorldFlag.createFlag(WorldFlag.MEMBERS_ONLY) else WorldFlag.createFlag(),
                activity = if (com.opennxt.config.Membership.enabled()) "Members" else "Free To Play",
                host = "127.0.0.1",
                playercount = 69
            ),
            WorldListEntry(
