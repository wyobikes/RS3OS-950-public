    ...
    fun checkFiles(type: String = "compressed", rsaConfig: RsaConfig = OpenNXT.rsaConfig) {
        ...
        if (!Files.exists(buildPath)) {
            ...
            throw FileNotFoundException(
                "No client files for build $latest at $buildPath (available: " +
                    "${if (staged.isEmpty()) "none" else staged.joinToString(", ")}). " +
                    "Run `run-tool client-downloader` or change the build in server.toml."
            )
        }
        ...
        if (verified.isEmpty())
            throw FileNotFoundException(
                "No binary type under ${Constants.CLIENTS_PATH.resolve(latest.toString())} has $type/jav_config.ws. " +
                        "Place a client under <type>/original/ and run `run-tool client-patcher`."
            )

        logger.info(
            "Client files OK for ${verified.joinToString(", ")}" +
                    if (absent.isEmpty()) "" else "; not present: ${absent.joinToString(", ")}"
        )
    }
