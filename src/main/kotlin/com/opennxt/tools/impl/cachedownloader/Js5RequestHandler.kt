...
class Js5RequestHandler(
    ...
    private val ioHighWater: Int =
        System.getProperty("opennxt.cachedl.iohighwater")?.toIntOrNull() ?: 8192
    ...
    override fun run() {
        ...
        try {
            while (running.get()) {
                ...
                synchronized(lock) {
                    val ioDepth = workers.sumOf { it.pendingOperations() }
                    if (ioDepth < ioHighWater) {
