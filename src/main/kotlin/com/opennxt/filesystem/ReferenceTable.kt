    ...
    fun archiveSize(): Int {
        if (mask and 0x4 != 0) {
            ...
            for (value in archives.values)
                sum += value.uncompressedSize.toLong()
            if (sum > Int.MAX_VALUE) {
                logger.warn {
                    "index $index: uncompressed size $sum exceeds int32, sent wrapped as ${sum.toInt()}"
                }
            }
            return sum.toInt()
        } else {
            ...
            for (key in archives.keys) {
                val data = filesystem.read(index, key)
                if (data == null) {
                    missing++
                    continue
                }
                sum += Container.decode(data).data.size.toLong()
            }
            if (missing > 0) {
                logger.warn {
                    "index $index: $missing of ${archives.size} archive(s) missing; reported size is a lower bound"
                }
            }
            return sum.toInt()
        }
